...
class DDPG:
    ...
    def update(self, transition_dict):
        ...
        actions = torch.tensor(transition_dict['actions'], dtype=torch.float).to(self.device) #注意这里是连续动作
        ...
        actor_loss = -torch.mean(self.critic(states, self.actor(states))) #策略网络就是为了使得Q值最大化
...
"定义一个特殊的经验回放池，词回放池不再存储每一步的数据，而是存储一整条轨迹。这是HER算法的核心部分，之后可以用HER算法从该经验回放池中构建新的数据来帮助策略训练"
class Trajectory:
    "用来记录一条完整的轨迹"
