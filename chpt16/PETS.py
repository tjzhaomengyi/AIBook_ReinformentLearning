...
import collections
from chpt16.cross_entropy import CEM
import itertools
...
"定义一个EnsembleDynamicsModel类，把模型集成的训练设计得更加精细化。具体而言，我们不会选择模型训练的轮数，而是在每次训练的时候将一部分" \
"数据单独取出来，用于验证模型的表现，在5次没有获得表现提升的时候就结束训练"
    ...
    def train(self, inputs, labels, batch_size=64, holdout_ratio=0.1, max_iter=20):
        ...
        for epoch in itertools.count():
            ...
            with torch.no_grad():
                ...
                break_condiction = self._save_best(epoch, holdout_losses)
                if break_condiction or epoch > max_iter:
                    break
...
"定义一个FakeEnv，用于实现给定状态和动作，用模型集成来进行预测，用在MPC算法中"
class FakeEnv:
...
"定义经验回访池的类Replay Buffer，此处经验回放缓冲区会额外实现一个返回所有数据的函数"
class ReplayBuffer:
...
class PETS:
    ...
    def mpc(self):
        ...
        while not done:
            ...
            action = actions[:self._action_dim] #选择第一个动作
    ...
    def train(self):
        ...
        for i_episode in range(self.num_episodes - 1):
            ...
            print('episoed: %d, return: %d' %(i_episode + 2, episode_return))
        return return_list
