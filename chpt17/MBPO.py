...
"和SAC算法相比，MBPO多用了一些模型推演得到的数据来训练策略"
#Actor
...
"定义环境模型，这里的环境模型和PETS算法中的环境模型是一样的，由多个高斯分布策略的集成来构建，沿用PETS算法中的模型构建"
    ...
    def update_agent(self, policy_train_batch_size = 64):
        ...
        for epoch in range(10):
            ...
            if self.model_pool.size() > 0:
                ...
                transition_dict = {'states': obs, 'actions': action, 'next_states': next_obs, 'reward': reward, 'dones':done}
